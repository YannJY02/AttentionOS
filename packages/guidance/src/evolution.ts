import type { WorkflowFacts } from '@attentionos/workflow';
import type {
...
import type { GuidanceSuggestion, V2AttentionObservationRecord } from './types';
...
export interface AnalyzeBehaviorPatternsInput {
  ...
  readonly suggestions: readonly GuidanceSuggestion<object>[];
...
export function measureSuggestionAdoption(
  suggestions: readonly GuidanceSuggestion<object>[],
): SuggestionAdoptionSummary {
...
export function analyzeBehaviorPatterns({
  ...
  const audit = workflow.auditEntries;
  const tasks = workflow.entities;
