...
export function calculateInterventionReward(outcome: {
  ...
  return Number(clamp(0.55 * focusComponent + 0.45 * switchComponent + acceptanceBoost, 0, 1).toFixed(4));
}
...
export function evaluateActionConfirmationPolicy(
  ...
  const explicitRisk = typeof payload.riskLevel === "string" ? payload.riskLevel.toLowerCase() : undefined;
